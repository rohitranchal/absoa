var debug = require('debug')('patient');
...
var req_key = ['ab.user.medical_data', 'ab.user.history'];

var patient_id = '001122';

/* GET home page. */
router.get('/', function(req, res) {
	res.send('Laboratory Service');
});

router.get('/pat_get', function(req, res) {
...
router.post('/pat_update', function(req, res) {
	...
	request.post({
		...
		if (err) {
			...
			if (body.search('EHR updated') != -1) {
				res.send('EHR sent');	
			} else {
				res.send('EHR sending failed');
			}
		}
	});
});
