...
/* Get latest logs for a list of services */
exports.get_service_list_log = function(services, cb) {
	var query = "SELECT * FROM Service_Log WHERE service_id IN (" + services + ")";
	debug('Get service list log: ' + query);
	connection.query(query, function(err, rows, fields) {
		if (err) throw err;
		cb(rows);
	});
